...
for (let key in data) {
  ...
  if (
    data[key].width > constants.RESIZE_IMAGE_WIDTH &&
    data[key].extension === ".png" &&
    !data[key].special
  ) {
    if (
      data[key].size > constants.MAX_HD_IMAGE_SIZE &&
      ...
      console.log("+++++++++++++++++++++++++");
      console.log(
        `Processing through JIMP + IMAGEMIN and resizing to ${constants.RESIZE_HD_IMAGE_WIDTH}px', ${data[key].filename}`
      );
      ...
      console.log("+++++++++++++++++++++++++");
      console.log(
        `Processing through JIMP + IMAGEMIN and resizing to ${constants.RESIZE_IMAGE_WIDTH}px', ${data[key].filename}`
      );
      Jimp.read(data[key].filepath, (err, lenna) => {
        if (err) throw err;
        lenna
  ...
  if (
    ...
    if (
      ...
      JimpResizeCompress(
        data[key].filepath,
        constants.RESIZE_HD_IMAGE_WIDTH,
        data[key].optimal
      );
    } else {
      JimpResizeCompress(
  ...
  if (data[key].width <= constants.RESIZE_IMAGE_WIDTH && !data[key].special) {
    if (
      data[key].size > constants.MAX_IMAGE_SIZE &&
      data[key].width > constants.RESIZE_HD_IMAGE_WIDTH
    ) {
...
function JimpResizeCompress(filepath, resize, destination) {
  ...
  console.log("+++++++++++++++++++++++++");
  console.log(
    `Processing through JIMP and resizing to ${resize}px', ${filepath}`
  );
